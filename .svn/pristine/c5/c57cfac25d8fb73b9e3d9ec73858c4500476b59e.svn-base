...
function _captureEnterToElement() { 
	...
	var button1 = null;
	var el;
	if (this.form["EnterButton"]!=null) {
		button=document.all(this.form["EnterButton"]);
	} else for (var i=0; i< this.form.elements.length; i++) {
...
//Sets the EnterButton expando property for the appropriate form to name.
//This allows capturing that button later.
function setFormEnterButton(name) {
		document.all(name).form["EnterButton"]=name;
}
